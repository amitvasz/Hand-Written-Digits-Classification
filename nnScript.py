...
def preprocess():
    ...
    train_eq_col = np.all(train_data == train_data[0,:],axis=0)

    temp_val = []
    k = 0
    for i in train_eq_col:
        if i == True:
            temp_val.append(k)
        else:
    ...
    train_data = np.delete(train_data,temp_val,1)
    validation_data = np.delete(validation_data,temp_val,1)
    test_data = np.delete(test_data,temp_val,1)
...
def nnObjFunction(params, *args):
    ...
    for i in range(trainingDataLength):
        OutputKSchemeMatrix[int(training_label[i])][i] = 1
    ...
    grad_w2=np.dot((OutputLayer-OutputKSchemeMatrix),HiddenLayerBiasAdded.transpose())
...
ab
    ...
    trainingDataLength = len(data)
    inputData = np.ones((trainingDataLength,len(data[0])+1))
    inputData[:,:-1] = data
    inputData = inputData.transpose()
    ...
    OutputLayer = np.zeros(shape=(10,trainingDataLength))
    OutputLayer = np.dot(w2,HiddenLayerBiasAdded)
    OutputLayer = sigmoid(OutputLayer)

    #Find maximum from each row
    labels = np.argmax(OutputLayer.transpose(),axis=1)
    return labels
